...
const sidebarOptions = {
  ...
  settings: [
    { label: "System Settings", name: "Settings.Index", icon: "fa-regular fa-gear" },
  ],
};
...
<template>
  <div v-if="sidebarStore.mobileDrawerVisible" class="fixed inset-0 z-[70] flex items-end justify-center pointer-events-none md:hidden">
    <!-- Overlay -->
    <div
      v-if="sidebarStore.mobileDrawerVisible"
      ...
      :class="sidebarStore.mobileDrawerVisible ? 'opacity-100' : 'opacity-0'"
    ></div>

    <!-- Drawer -->
    <div
      class="relative w-[95%] max-w-md mx-auto bg-white rounded-t-3xl shadow-2xl pointer-events-auto overflow-hidden"
      :style="drawerStyle"
    >
      <!-- Draggable Header Section (Handle bar + Header) -->
      <div
        class="cursor-grab active:cursor-grabbing touch-none select-none flex-shrink-0"
      ...
      >
        ...
        <div class="px-5 pb-3 border-b border-gray-200">
          <div class="flex items-center gap-3">
            <div class="w-10 h-10 rounded-xl bg-gradient-to-tr from-[#6D28D9] to-[#2563EB] flex items-center justify-center text-white font-semibold text-sm shadow-sm">
              Dorex
            </div>
            <div>
      ...
      <!-- Content -->
      <div class="overflow-y-auto px-5" :style="{ height: `calc(${drawerHeight}vh - 100px)`, maxHeight: `calc(${drawerHeight}vh - 100px)`, touchAction: 'pan-y' }">
        <div class="flex flex-col pt-4 pb-20">
          <!-- Sidebar Items Grid (3 per row) -->
          <div v-if="currentMenuOptions.length > 0" class="mb-4">
            <div class="grid grid-cols-3 gap-3">
              <button
                v-for="option in currentMenuOptions"
                :key="option.name"
                @click="navigateToRoute(option.name)"
                :class="[
