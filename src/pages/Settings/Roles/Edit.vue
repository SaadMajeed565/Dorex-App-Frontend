...
const props = defineProps<{ visible: boolean; roleId: string | number | null }>();
...
const submitForm = async () => {
  ...
  loading.value = true;
  try {
    const response = await axiosClient.put(`/roles/${roleForm.value.id}`, {
    ...
    toast.add({
      severity: 'success',
      summary: 'Success',
      detail: response.data.message || 'Role updated successfully!',
      life: 3000,
    });
    ...
    toast.add({
      severity: 'error',
      summary: 'Error',
      detail: error.response?.data?.message || 'Failed to update role. Please try again.',
      life: 4000,
    });
  } finally {
    loading.value = false;
